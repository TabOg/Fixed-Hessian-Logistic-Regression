...
int Fixed_Hessian_IDASH() {
    ...
    dMat Matrix;
    ImportDataLR(Matrix, "idash.txt", true, 2, ',');

    int n = Matrix.size();

    int nfeatures = Matrix[0].size();

    cout << "n =" << n << ", nfeatures = " << nfeatures << "\n";
    ...
    Matrix.clear();
    cout << "2";
    ImportDataLR(Matrix, "idash.txt", true, 1, ',');
    cout << "3";
    cout << "weights size is: " << weights.size() << "\n";
    cout << "sample size is: " << Matrix[10].size() << "\n";
